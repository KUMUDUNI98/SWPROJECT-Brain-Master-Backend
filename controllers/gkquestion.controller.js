...
function save(req, res) {
    ...
    models.GK_Question.create(question)
        .then((result) => {
            res.status(201).json({//successfull responces
                message: "GK Question created successfully",
                question: result,
            });
        })
        .catch((error) => {// server error catch
            res.status(500).json({
...
function index(req, res) {//all find queston
...
function destroy(req, res) {
    ...
    models.GK_Question.destroy({where: {question_id: id}})//delete
        .then((result) => {
            if(result){
                res.status(200).json({
